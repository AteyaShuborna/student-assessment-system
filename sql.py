...
def insert_into_student(student_id, name, email, password, administer,status):
    try:
        connection = mysql.connector.connect(host='localhost',
                                             database='cse370_project',
                                             user='root',
        ...
        # Insert a record
        mySql_insert_query = """INSERT INTO student (student_id, name, email, password, administer,status)
                                VALUES (%s, %s, %s, %s, %s,%s) """

        record = (student_id, name, email, password, administer,status)
        cursor.execute(mySql_insert_query, record)
        connection.commit()
        print("Record inserted successfully into Laptop table")
...
def delete_enroll(student_id, course, semester):
    try:
        connection = mysql.connector.connect(host='localhost',
                                            database='cse370_project',
                                            user='root')
        ...
        # Delete a record
        sql_Delete_query = f"DELETE FROM enrolled_courses WHERE (course='{course}') AND (student_id = '{student_id}') ;"
...
#delete assessment
def del_assessment_db(course, semester, type):
    try:
        connection = mysql.connector.connect(host='localhost',
                                             database='cse370_project',
                                             user='root',
        ...
        # Delete a record
        sql_Delete_query = f"DELETE FROM assesment WHERE (course = '{course}') AND (semester='{semester}') AND (type='{type}');"
        print(sql_Delete_query)
        cursor.execute(sql_Delete_query)
        connection.commit()
        print('number of rows deleted', cursor.rowcount)
